...
const calculations = async function (user, items, res) {
  if (items[0].length == 0) {
    ...
    for(let item of rest.items) {
      ...
      await orderDetails.itemDetails(name, category, imported, quantity, price, finalPrice, salesTax);
    }
...
async function calcPrice(items, index, tax, total, res) {
  if(items[index] != undefined) {
    ...
    let category = isFood ? `food` : isMedicine ? `medicine` : isBook ? `book` : `other`;
    let finalPrice = quantity * price;
    let salesTax = 0;
    if(imported) {
      salesTax = salesTax + (finalPrice * 0.05);
    ...
    await orderDetails.itemDetails(product, category, imported, quantity, price, finalPrice, salesTax);
    res.write(`${item}\n`);
