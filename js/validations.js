firstName.onblur=function errorMessage() {
  if(firstName.value.length==0) errorMessageFirstName.style.display='block';
}
...
lastName.onblur=function errorMessage() {
  if(lastName.value.length==0) errorMessageLastName.style.display='block';
}
...
company.onblur=function errorMessage() {
  if(company.value.length==0) errorMessageCompany.style.display='block';
}
...
areaCode.onblur=function errorMessage() {
  // body...
  if(areaCode.value.length==0) errorMessageAreaCode.style.display='block';
}
...
phone.onblur=function errorMessage() {
  // body...
  if(phone.value.length==0) errorMessagePhone.style.display='block';
}
